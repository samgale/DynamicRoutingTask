...
modelParams = []
configs = []
for mouseId,sessions in zip(mouseIds[:1],sessionStartTimes[:1]):
    sessionData = [getSessionData(mouseId,st) for st in sessions[:21]]
    testIndex = [2]
    ...
    disrnn_config = disrnn.DisRnnConfig(
        ...
        update_net_obs_penalty=0.001, # 0.01
    ...
    configs.append(disrnn_config)


params = modelParams[0]
disrnn_config = configs[0]
