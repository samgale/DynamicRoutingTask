    ...
    for mouseId in mouseIds:
        ...
        for obj in exps:
            ...
            if 'stage' in obj.taskVersion and 'templeton' not in obj.taskVersion:
                    ...
                    if 'stage 1' in task:
                        ...
                        if ('stage 3' in prevTask
                             and ((regimen==1 and all(all(h > hitThresh for h in hc) for hc in hits) and all(all(d > dprimeThresh for d in dp) for dp in dprimeSame))
                                  or (regimen in (2,3,4) and all(all(h > hitThresh/2 for h in hc) for hc in hits) and all(all(d > dprimeThresh for d in dp) for dp in dprimeSame+dprimeOther)))):
                            passStage = 1
                            ...
                            elif (regimen==2 and not any('stage 3 tone' in s for s in df['task version'])) or regimen in (3,4):
                                nextTask = 'stage 3 ori'
                            else:
                ...
                if 'stage 3' in nextTask and regimen in (2,3,4):
                    nextTask += ' distract'
