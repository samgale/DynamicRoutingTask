...
class DynRoutData():
    ...
    def loadBehavData(self,filePath,h5pyFile=None,engagedThresh=None,lightLoad=False):
        ...
        with context:
            ...
            self.trialHasOptoQuiescentViolation = d['trialHasOptoQuiescentViolation'][:self.nTrials] if 'trialHasOptoQuiescentViolation' in d.keys() else None
            ...
            try:
                ...
                if 'stage' in obj.taskVersion and 'templeton' not in obj.taskVersion:
                    ...
                    if regimen>3 and 'stage 2' not in nextTask and nextTask != 'hand off':
                        nextTask += ' moving'
                    if not handOff and allMiceDf.loc[mouseInd,'timeouts'] and 'stage 0' not in nextTask and ((regimen>3 and 'timeouts' in task) or 'stage 5' not in nextTask):
                        nextTask += ' timeouts'
