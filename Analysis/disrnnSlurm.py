...
dirName = 'reps' # 'penalties', 'reps', or 'unpooledGrus'
if dirName in ('penalites','reps'):
    nProcesses,nReps = (24,3) if dirName == 'penalties' else (1,64)
...
for mouseId in mouseIds:
    for trainingPhase in trainingPhases:
        for rep in range(nReps):
            slurm.sbatch('{} {} --dirName {} --rep {} --nProcesses {} --trainingPhase {} --mouseId {} --maxTrainSessions {}'.format(
                         python_path,script_path,dirName,rep,nProcesses,trainingPhase,mouseId,maxTrainSessions))
