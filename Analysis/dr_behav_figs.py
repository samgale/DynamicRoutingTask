...
mice = {'NSB': np.array(summaryDf[stage1Pass & isNsb]['mouse id']),
        'DR': np.array(summaryDf[stage1Pass & ~isNsb]['mouse id'])}
plotLearning(mice,stage=1,xlim=None)

stage2Pass = isStandardRegimen & summaryDf['stage 2 pass']
mice = {'NSB': np.array(summaryDf[stage2Pass & isNsb]['mouse id']),
        'DR': np.array(summaryDf[stage2Pass & ~isNsb]['mouse id'])}
plotLearning(mice,stage=2,xlim=None)

stage5Pass = isStandardRegimen & summaryDf['stage 5 pass']
mice = {'NSB': np.array(summaryDf[stage5Pass & isNsb]['mouse id']),
        'DR': np.array(summaryDf[stage5Pass & ~isNsb]['mouse id'])}
...
for exps,sp in zip(sessionData,sessionsToPass):
    ...
    for obj in exps[:sp+5]:
        ...
        if obj.blockStimRewarded[0] == 'vis1':
            dv[-1].append(np.nanmean(obj.dprimeOtherModalGo[::2]))
            da[-1].append(np.nanmean(obj.dprimeOtherModalGo[1::2]))
        else:
            da[-1].append(np.nanmean(obj.dprimeOtherModalGo[::2]))
            dv[-1].append(np.nanmean(obj.dprimeOtherModalGo[1::2]))
...
smoothSigma = 1
dp,dv,da,rr = [[scipy.ndimage.gaussian_filter(a,smoothSigma) for a in b] for b in (dp,dv,da,rr)]
...
for side in ('right','top'):
    ax.spines[side].set_visible(False)
ax.tick_params(direction='out',top=False,right=False,labelsize=12)
ax.set_xlabel('Cross-modal d\' vis blocks',fontsize=14)
ax.set_ylabel('Cross-modal d\' aud blocks',fontsize=14)
plt.tight_layout()
...
ax.tick_params(direction='out',top=False,right=False,labelsize=12)
ax.set_xlim([-65,65])
ax.set_xlabel('Lag between cross-modal d\' for aud and vis blocks',fontsize=14)
ax.set_ylabel('Number of mice',fontsize=14)
plt.tight_layout()
...
i = 0
j = 0
for vis,aud in zip(dv,da):
    if j==ncols:
        i += 1
        j = 0
    ax = fig.add_subplot(gs[i,j])
    j += 1
    for a,clr in zip((vis,aud),'gm'):
        y = np.array(a)
        y -= y.min()
        y /= y.max()
        x = np.arange(len(y))+1
...
respTimePrevNoRew = copy.deepcopy(respNext)
        ...
        for rewardStim,blockType in zip(('all','vis1','sound1'),('all','visual','auditory')):
            for s in stimTypes:
                for exps,sp in zip(sessionData,sessionsToPass):
                        ...
                        for blockInd,rewStim in enumerate(obj.blockStimRewarded):
                            if rewardStim in ('all',rewStim):
                                ...
                                if False:
                                    ...
                                    rpnr.append(obj.trialResponse[trials][prevInd[trials+1] & ~obj.trialRewarded[trials-1]])
                                    rtpnr.append(obj.responseTimes[trials][prevInd[trials+1] & ~obj.trialRewarded[trials-1]])
                    ...
                    respPrevNoRew[phase][prevTrialType][blockType][s].append(np.nanmean(rpnr))
                    respTimePrevNoRew[phase][prevTrialType][blockType][s].append(np.nanmean(rtpnr))
    ...
    for phase in ('initial training','after learning'):
        ...
        for i,stim in enumerate(stimTypes):
            for j,prevTrialType in enumerate(prevTrialTypes):
                r[i,j] = np.nanmean(np.array(respNext[phase][prevTrialType][blockType][stim]) - np.array(d[phase][prevTrialType][blockType][stim]))
        fig = plt.figure(figsize=(10,8))
        ax = fig.add_subplot(1,1,1)
